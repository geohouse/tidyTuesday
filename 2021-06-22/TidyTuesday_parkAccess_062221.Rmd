...
#### Considering all park features together, are there characteristics of parks in some cities or some states that make them unique compared to parks in other cities or states? I'll use principal coordinates analysis (PCoA) to visualize differences in the different park characteristics among all cities and states, and then look at which park features are prominent in the loadings of the PCoA axes to see what park features drive these differences.

```{r add state to city data}
...
all_data_add_pct$state <- purrr::map(.x = all_data_add_pct$city, .f = mapCityToState)
